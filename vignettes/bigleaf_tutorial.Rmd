...
This vignette is a short introduction to the functionalities of the `bigleaf` R package (version 0.6.5). It is directed to first-time package users who are familiar with the basic concepts of R. After presenting the use of several key functions of the package, some useful hints and guidelines are given at the end of the vignette.
...
We give the data.frame a shorter name here. More information on the data (e.g. meaning of column names and units) can be found when typing `?DE_Tha_Jun_2014`. For more information on the site see e.g. Grünwald & Bernhofer 2007.
In addition, we will need some ancillary data on the site throughout this tutorial. To ensure consistency, we define them here at the beginning:
...
```{r}
## stomatal slope from the USO model (Medlyn et al. 2011)
g1_USO <- stomatal.slope(tha_filtered,Tair="Tsurf",VPD="VPD_surf",Ca="Ca_surf",model="USO",g0=0,robust.nls=TRUE)
g1_USO
```
...
plot(tha_filtered[,"Gs_mol"] ~ stomatal_index,las=1,xlab=expression("GPP / (C"["a"]~sqrt("D"["a"])*")"),
     ylab=expression("G"["s,w"]~"(mol m"^{-2}~"s"^{-1}*")"),tcl=0.2,mgp=c(2.2,0.5,0),xlim=c(0,0.12))
```
...
In `bigleaf`, a wind profile can be calculated assuming an exponential increase with height, which is affected by atmospheric stability. Here, we calculate wind speed at heights of 22-60m in steps of 2m. As expected, the gradient in wind speed is strongest close to the surface and weaker at greater heights:
...
plot(wind_heights ~ wp_means,xlab=expression("wind speed (m s"^{-1}*")"),ylab="height (m)",las=1,
     mgp=c(2.2,0.5,0),tcl=0.2,xlim=c(0,5))
...
```
Here, the points denote the mean wind speed and the bars denote the standard deviation. The blue point/bars represent the values that were measured at 42m. In this case we see that the wind speed "back-calculated" from the wind profile agrees well with the actual measurements.
...
The blue point shows the estimate (+/- standard error) when we take all data, as calculated above. The two estimates agree well, but we are likely to overestimate $g_1$ slightly if we do not apply bootstrapping. The bootstrapping technique can also be applied to other (regression-based) functions in the package.
...
In the example above we create a parameter space that we use for the subsequent calculations. We have chosen the most simple settings here, that means we assume that parameters have a normal error distribution and that they are independent of each other. In many cases these assumptions are not valid. For example, measured fluxes are more likely to have a Laplace error distribution (Hollinger & Richardson 2005), which would be better sampled using `rlaplace` from the `rmutil` package instead of `rnorm`. In many cases, the parameters are also not independent of each other. In our case, $z_{0,m}$ and $D_l$ may not be strongly correlated, but one would possibly expect a correlation between LAI and $z_{0,m}$. We can account for dependencies among variables by doing the sampling based on a variance-covariance matrix that prescribes correlations between variables.
...
plot(Ga_mean[ind],type="l",lwd=2,xlab="timestep",ylab=expression("G"["a,h"]~"(m s"^{-1}*")"),las=1,
     mgp=c(2.2,0.5,0),tcl=-0.2,ylim=c(0.045,0.14))
...
plot(Gs_mean[ind],type="l",lwd=2,xlab="timestep",ylab=expression("G"["s,w"]~"(mol m"^{-2}~"s"^{-1}*")"),
     las=1,mgp=c(2.2,0.5,0),tcl=-0.2)
